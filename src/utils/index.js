...
export const axiosInstance = (() => {
  ...
  _instance.interceptors.request.use(
    async (configs) => {
      ...
      if (!accessToken || Number(jwtDecode(accessToken).exp) <= Math.floor(Date.now() / 1000) + 300) {
        // Access token expired
        const refreshToken = localStorage.getItem("refreshToken")
        if (!refreshToken || Number(jwtDecode(refreshToken).exp) <= Math.floor(Date.now() / 1000) + 300) {
          // Session expired
          return configs
        }
        try {
          const { data: { accessToken: newAccessToken } } = await axios({
            method: "POST",
